...
10. 仕事内容:
    - 「仕事内容」「■業務内容」フィールドの値を基に、**必ず**文章と箇条書きを組み合わせて生成してください
    - **【必須形式】**
      1) 冒頭に2〜3行のリード文で仕事の全体像・やりがい・どんな人に向いているかを、きちんとしていながら親しみやすい口調で説明してください
      2) その後に、具体的な業務内容を箇条書き（・を使用）で3〜6個リストアップしてください（1項目は1〜2行で簡潔に）
    - 視認性を高めるため、箇条書きの前後に改行を入れてください
    - トーンは「きちんとしているが、固すぎず、求職者に話しかけるような親しみやすさ」を意識してください（例：「〜していただきます」よりも「〜するお仕事です」のような表現）
    - 専門用語には簡単な説明を加えるなど、初めての人にも伝わる親切な表現を心がけてください
    - 例：
      「大手食品工場での簡単な検品作業をお任せします。
      ラインから流れてくる製品にキズや汚れがないかを目視でチェックするシンプルなお仕事です。
      
      ・製品の目視検査（キズ・汚れのチェック）
      ・不良品の選別作業
      ・梱包済み製品のラベル確認
      
      未経験の方でも、先輩スタッフが丁寧に指導しますのでご安心ください。」
...
12. 待遇:
    - 「待遇」フィールドから福利厚生・各種手当・ボーナス・昇給など、働くうえでの安心材料となる情報を整理して記載してください
    - **【推奨形式】**
      1) 冒頭に1〜2行で「どんな待遇面が魅力か」をまとめた文章（例：「安定して長く働けるよう、福利厚生も充実しています。」）
      2) その後、以下のような項目を箇条書きで整理してください（該当するもののみでOK）：
         ・社会保険・雇用保険などの加入状況  
         ・交通費・各種手当（残業手当、深夜手当、役職手当 等）  
         ・ボーナス（支給有無、回数、目安）  
         ・昇給（頻度・評価タイミング）  
         ・休憩室・ロッカー・社員食堂などの設備  
         ・研修・資格支援などの成長サポート
    - 文体は「安心感があり、かつ押しつけがましくない」親しみやすいトーンにしてください（例：「〜が整っています」「〜も利用できます」など）
...
20. 備考（外部非公開情報はここに記載）:
   - Excelデータに内部情報や補足情報があれば記載してください
   - 情報がない場合は必ず空文字列（""）にしてください
   - 【除外】「企業名公表不可」やそれに類する文言は不要ですので、含めないでください

【重要】
- Excelデータに存在する情報のみを抽出してください
- 情報がない項目は必ず空文字列（""）にしてください
- フェーズ1で抽出されたテキスト情報を**最大限活用**し、情報を省略・要約しすぎないでください。
- 特に「仕事内容」や「キャッチコピー」は、元のテキストに含まれる詳細なニュアンスや魅力を損なわないよう、リッチな表現を維持してください。
- フェーズ1のテキストが非常に詳細であれば、その詳細さを保ったまま各項目に振り分けてください。簡素化しすぎて情報が失われないように注意してください。
