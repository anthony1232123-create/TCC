...
10. 仕事内容:
    ...
    - 箇条書き（・）を活用して、視認性を高めてください
    - Excelデータの内容を整理して、求職者がイメージしやすい魅力的な文章にしてください
...
- 読みやすい形式で整理してください
- Excelデータに記載されているすべての情報を含めてください

【重要なルール】
1. Excelデータに記載されている情報をすべて抽出してください（漏れなく）
2. データが「|」で区切られている場合：
   - 最初の値が項目名、2番目以降が値です
   - 複数の値がある場合は、すべての値を含めてください
3. プルダウン形式の項目の処理：
   - 値が空欄の場合（「（記載なし）」と表示されている場合）：
     * 「勤務時間帯」「長期休暇」など：空欄で出力（「項目名: （記載なし）」として記載）
     * 「ここがポイント」：全て空欄の場合、「ここがポイントに記載はありませんでした。」と記載
   - 値がある場合は通常通り「項目名: 値」として記載
4. 推測や補完は行わず、Excelデータに記載されている情報のみを使用してください
5. 時間の表記（例：8:30、17:00）はそのまま保持してください
6. 「休日」という項目で「年間休日」という値がある場合は、「年間休日: 値」として記載してください
7. 全ての項目について、目を通したことがわかるように記載してください（値が空欄でも項目名は記載）
8. 全ての情報を漏れなく含めてください
9. 【除外項目】以下の項目は出力に含めないでください（内部情報のため）：
   - 営業所名、担当者
   - 企業名、正式名称、企業名(正式名称)
   - 案件名
   - 企業ホームページURL
10. 【重要】数値データ（回数、金額、人数など）は絶対に省略せず、正確に記載してください（例：ボーナス2回、昇給年1回など）`;
...
- 全ての項目について、目を通したことがわかるように記載してください（値が空欄でも項目名は記載）
